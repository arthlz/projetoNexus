...
def mock_env_settings():
    ...
    with patch("backend.services.llm_service.get_settings", return_value=mock_config), \
...
async def test_chamar_llm_sucesso(mock_openai_class):
    ...
    mock_response.choices = [MagicMock(message=MagicMock(content="Resposta simulada de entrevista."))]
...
async def test_chamar_llm_falha_total(mock_openai_class):
    ...
    mock_client_instance.chat.completions.create.side_effect = Exception("API fora do ar")
...
def test_transcrever_audio_sucesso(mock_deepgram_class):
    ...
    mock_response.results.channels = [
        MagicMock(alternatives=[MagicMock(transcript="Esta é a resposta do candidato.")])
    ]
